...
public class OrbitParticleAir implements ParticleOptions {
    public final Vec3 center;
    public final double omegaRadPerSec;
    public final int    lifetimeTicks;


    public OrbitParticleAir(Vec3 center, double omegaRadPerSec, int lifetimeTicks) {
        this.center = center;
        this.omegaRadPerSec = omegaRadPerSec;
        this.lifetimeTicks = lifetimeTicks;
    }

    public static final Codec<OrbitParticleAir> CODEC =
            ...
            ).apply(i, OrbitParticleAir::new));

    public static final Deserializer<OrbitParticleAir> DESERIALIZER =
            new Deserializer<>() {
                @Override
                public OrbitParticleAir fromCommand(ParticleType<OrbitParticleAir> type, StringReader reader)
                    ...
                    return new OrbitParticleAir(new Vec3(cx, cy, cz), omega, life);
                }

                @Override
                public OrbitParticleAir fromNetwork(ParticleType<OrbitParticleAir> type, FriendlyByteBuf buf) {
                    double cx = buf.readDouble();
                    double cy = buf.readDouble();
                    double cz = buf.readDouble();
                    double om = buf.readDouble();
                    int life = buf.readVarInt();
                    return new OrbitParticleAir(new Vec3(cx, cy, cz), om, life);
                }
            };
    ...
    public ParticleType<?> getType() {
        return ModParticles.SKILL36_Particle.get();
    }
}