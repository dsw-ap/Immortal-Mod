...
public class OrbitParticleDarkness implements ParticleOptions {
    public final Vec3 center;
    public final double omegaRadPerSec;
    public final int    lifetimeTicks;


    public OrbitParticleDarkness(Vec3 center, double omegaRadPerSec, int lifetimeTicks) {
        this.center = center;
        this.omegaRadPerSec = omegaRadPerSec;
        this.lifetimeTicks = lifetimeTicks;
    }

    public static final Codec<OrbitParticleDarkness> CODEC =
            ...
            ).apply(i, OrbitParticleDarkness::new));

    public static final Deserializer<OrbitParticleDarkness> DESERIALIZER =
            new Deserializer<>() {
                @Override
                public OrbitParticleDarkness fromCommand(ParticleType<OrbitParticleDarkness> type, StringReader reader)
                    ...
                    return new OrbitParticleDarkness(new Vec3(cx, cy, cz), omega, life);
                }

                @Override
                public OrbitParticleDarkness fromNetwork(ParticleType<OrbitParticleDarkness> type, FriendlyByteBuf buf) {
                    double cx = buf.readDouble();
                    double cy = buf.readDouble();
                    double cz = buf.readDouble();
                    double om = buf.readDouble();
                    int life = buf.readVarInt();
                    return new OrbitParticleDarkness(new Vec3(cx, cy, cz), om, life);
                }
            };
    ...
    public ParticleType<?> getType() {
        return ModParticles.SKILL66_Particle.get();
    }
}