...
public class OrbitParticleWater_ implements ParticleOptions {
    public final Vec3 center;
    public final double omegaRadPerSec;
    public final int    lifetimeTicks;


    public OrbitParticleWater_(Vec3 center, double omegaRadPerSec, int lifetimeTicks) {
        this.center = center;
        this.omegaRadPerSec = omegaRadPerSec;
        this.lifetimeTicks = lifetimeTicks;
    }

    public static final Codec<OrbitParticleWater_> CODEC =
            ...
            ).apply(i, OrbitParticleWater_::new));

    public static final Deserializer<OrbitParticleWater_> DESERIALIZER =
            new Deserializer<>() {
                @Override
                public OrbitParticleWater_ fromCommand(ParticleType<OrbitParticleWater_> type, StringReader reader)
                    ...
                    return new OrbitParticleWater_(new Vec3(cx, cy, cz), omega, life);
                }

                @Override
                public OrbitParticleWater_ fromNetwork(ParticleType<OrbitParticleWater_> type, FriendlyByteBuf buf) {
                    double cx = buf.readDouble();
                    double cy = buf.readDouble();
                    double cz = buf.readDouble();
                    double om = buf.readDouble();
                    int life = buf.readVarInt();
                    return new OrbitParticleWater_(new Vec3(cx, cy, cz), om, life);
                }
            };
    ...
    public ParticleType<?> getType() {
        return ModParticles.SKILL26_Particle.get();
    }
}