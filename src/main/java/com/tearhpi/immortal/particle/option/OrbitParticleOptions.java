...
public class OrbitParticleOptions implements ParticleOptions {
    public final Vec3 center;
    public final double omegaRadPerSec;
    public final int    lifetimeTicks;

    public OrbitParticleOptions(Vec3 center, double omegaRadPerSec, int lifetimeTicks) {
        this.center = center;
        this.omegaRadPerSec = omegaRadPerSec;
        this.lifetimeTicks = lifetimeTicks;
    }

    public static final Codec<OrbitParticleOptions> CODEC =
            ...
            ).apply(i, OrbitParticleOptions::new));

    public static final Deserializer<OrbitParticleOptions> DESERIALIZER =
            new Deserializer<>() {
                @Override // /particle 命令里解析（可简单做个空实现或自己定义）
                public OrbitParticleOptions fromCommand(ParticleType<OrbitParticleOptions> type, StringReader reader)
                    ...
                    return new OrbitParticleOptions(new Vec3(cx, cy, cz), omega, life);
                }

                @Override
                public OrbitParticleOptions fromNetwork(ParticleType<OrbitParticleOptions> type, FriendlyByteBuf buf) {
                    double cx = buf.readDouble();
                    double cy = buf.readDouble();
                    double cz = buf.readDouble();
                    double om = buf.readDouble();
                    int life = buf.readVarInt();
                    return new OrbitParticleOptions(new Vec3(cx, cy, cz), om, life);
                }
            };
    ...
    public ParticleType<?> getType() {
        return ModParticles.SKILL11_Particle.get();
    }
}