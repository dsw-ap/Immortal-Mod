...
public class OrbitParticleEarth implements ParticleOptions {
    public final Vec3 center;
    public final double omegaRadPerSec;
    public final int    lifetimeTicks;


    public OrbitParticleEarth(Vec3 center, double omegaRadPerSec, int lifetimeTicks) {
        this.center = center;
        this.omegaRadPerSec = omegaRadPerSec;
        this.lifetimeTicks = lifetimeTicks;
    }

    public static final Codec<OrbitParticleEarth> CODEC =
            ...
            ).apply(i, OrbitParticleEarth::new));

    public static final Deserializer<OrbitParticleEarth> DESERIALIZER =
            new Deserializer<>() {
                @Override
                public OrbitParticleEarth fromCommand(ParticleType<OrbitParticleEarth> type, StringReader reader)
                    ...
                    return new OrbitParticleEarth(new Vec3(cx, cy, cz), omega, life);
                }

                @Override
                public OrbitParticleEarth fromNetwork(ParticleType<OrbitParticleEarth> type, FriendlyByteBuf buf) {
                    double cx = buf.readDouble();
                    double cy = buf.readDouble();
                    double cz = buf.readDouble();
                    double om = buf.readDouble();
                    int life = buf.readVarInt();
                    return new OrbitParticleEarth(new Vec3(cx, cy, cz), om, life);
                }
            };
    ...
    public ParticleType<?> getType() {
        return ModParticles.SKILL48_Particle.get();
    }
}