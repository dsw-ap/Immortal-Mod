...
public class OrbitParticleLight implements ParticleOptions {
    public final Vec3 center;
    public final double omegaRadPerSec;
    public final int    lifetimeTicks;


    public OrbitParticleLight(Vec3 center, double omegaRadPerSec, int lifetimeTicks) {
        this.center = center;
        this.omegaRadPerSec = omegaRadPerSec;
        this.lifetimeTicks = lifetimeTicks;
    }

    public static final Codec<OrbitParticleLight> CODEC =
            ...
            ).apply(i, OrbitParticleLight::new));

    public static final Deserializer<OrbitParticleLight> DESERIALIZER =
            new Deserializer<>() {
                @Override
                public OrbitParticleLight fromCommand(ParticleType<OrbitParticleLight> type, StringReader reader)
                    ...
                    return new OrbitParticleLight(new Vec3(cx, cy, cz), omega, life);
                }

                @Override
                public OrbitParticleLight fromNetwork(ParticleType<OrbitParticleLight> type, FriendlyByteBuf buf) {
                    double cx = buf.readDouble();
                    double cy = buf.readDouble();
                    double cz = buf.readDouble();
                    double om = buf.readDouble();
                    int life = buf.readVarInt();
                    return new OrbitParticleLight(new Vec3(cx, cy, cz), om, life);
                }
            };
    ...
    public ParticleType<?> getType() {
        return ModParticles.SKILL55_Particle.get();
    }
}