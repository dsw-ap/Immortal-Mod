...
public class OrbitParticleAir_vertical_ implements ParticleOptions {
    //center:平面锚定坐标
    //omegaRadPerSec:初始相位
    ...
    public OrbitParticleAir_vertical_(Vec3 center, double omegaRadPerSec, int lifetimeTicks) {
        this.center = center;
        this.omegaRadPerSec = omegaRadPerSec;
        this.lifetimeTicks = lifetimeTicks;
    }

    public static final Codec<OrbitParticleAir_vertical_> CODEC =
            ...
            ).apply(i, OrbitParticleAir_vertical_::new));

    public static final Deserializer<OrbitParticleAir_vertical_> DESERIALIZER =
            new Deserializer<>() {
                @Override
                public OrbitParticleAir_vertical_ fromCommand(ParticleType<OrbitParticleAir_vertical_> type, StringReader reader)
                    ...
                    return new OrbitParticleAir_vertical_(new Vec3(cx, cy, cz), omega, life);
                }

                @Override
                public OrbitParticleAir_vertical_ fromNetwork(ParticleType<OrbitParticleAir_vertical_> type, FriendlyByteBuf buf) {
                    double cx = buf.readDouble();
                    double cy = buf.readDouble();
                    double cz = buf.readDouble();
                    double om = buf.readDouble();
                    int life = buf.readVarInt();
                    return new OrbitParticleAir_vertical_(new Vec3(cx, cy, cz), om, life);
                }
            };
    ...
    public ParticleType<?> getType() {
        return ModParticles.SKILL36_Particle_vert_.get();
    }
}