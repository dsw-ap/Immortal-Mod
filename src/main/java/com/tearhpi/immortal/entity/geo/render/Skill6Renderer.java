...
import com.tearhpi.immortal.entity.geo.model.Skill6Model;
...
public class Skill6Renderer extends GeoEntityRenderer<Skill6_Entity> {
    private static final String WALL_BONE_NAME = "bone";
    public Skill6Renderer(EntityRendererProvider.Context context) {
        super(context,new Skill6Model());
        this.shadowRadius = 0.5f;
        this.addRenderLayer(new GlowLayer(this));
    }
    @Override
    public RenderType getRenderType(Skill6_Entity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(texture);
    }
    @Override
    public void render(Skill6_Entity entity, float entityYaw, float partialTick,
    ...
    public void actuallyRender(PoseStack poseStack,
        ...
        float base = 1.0f;
        float amp  = 0.1f;
        float scale = (float) (base * (1-animatable.getRenderScale()*0.8));
        scale = Mth.clamp(scale, 0.05f, 20.0f);
        float pivotY = 2.0f;
        ...
        if (!isReRender) {
            ...
            AnimationState<Skill6_Entity> animationState =
            ...
            GeoModel<Skill6_Entity> currentModel = this.getGeoModel();
    ...
    private static class GlowLayer extends GeoRenderLayer<Skill6_Entity> {
        public GlowLayer(Skill6Renderer renderer) {
            super(renderer);
        }

        @Override
        public void render(PoseStack poseStack, Skill6_Entity animatable, BakedGeoModel bakedModel, RenderType baseRenderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
