...
public class WandNormalAttackBulletRenderer  extends EntityRenderer<WandNormalAttackBullet> {
    private static final ResourceLocation TEXTURE =
            ResourceLocation.fromNamespaceAndPath("immortal", "textures/entity/skill8_entity_.png");


    public WandNormalAttackBulletRenderer(EntityRendererProvider.Context ctx) {
        super(ctx);
    }

    @Override
    public void render(WandNormalAttackBullet entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        ...
        if (attr == 1) {
            renderSinglePart(poseStack,buffer,TEXTURE,partialTick,0.1F,tick,0.2f,0.25f,normal_line,1.0f,0.0f,0.0f);
        } else if (attr == 2) {
            renderSinglePart(poseStack,buffer,TEXTURE,partialTick,0.1F,tick,0.2f,0.25f,normal_line,0.0f,0.0f,1.0f);
        } else if (attr == 3) {
            renderSinglePart(poseStack,buffer,TEXTURE,partialTick,0.1F,tick,0.2f,0.25f,normal_line,1.0f,1.0f,1.0f);
        } else if (attr == 4) {
            renderSinglePart(poseStack,buffer,TEXTURE,partialTick,0.1F,tick,0.2f,0.25f,normal_line,0.58f,0.58f,0.286f);
        } else if (attr == 5) {
            renderSinglePart(poseStack,buffer,TEXTURE,partialTick,0.1F,tick,0.2f,0.25f,normal_line,1.0f,1.0f,0f);
        } else if (attr == 6) {
            renderSinglePart(poseStack,buffer,TEXTURE,partialTick,0.1F,tick,0.2f,0.25f,normal_line,0.4f,0.0f,0.43f);
        } else {
            renderSinglePart(poseStack,buffer,TEXTURE,partialTick,0.1F,tick,0.2f,0.25f,normal_line,0.724f,0.657f,0.501f);
        }
    ...
    @Override
    public @NotNull ResourceLocation getTextureLocation(@NotNull WandNormalAttackBullet p_114482_) {
        return TEXTURE;
    }
}
