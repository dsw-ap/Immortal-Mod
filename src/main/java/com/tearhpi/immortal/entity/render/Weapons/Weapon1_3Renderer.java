package com.tearhpi.immortal.entity.render.Weapons;
...
public class Weapon1_3Renderer extends EntityRenderer<Weapon1_3_Entity> {

    private static final ResourceLocation TEXTURE =
            ResourceLocation.fromNamespaceAndPath("immortal", "textures/entity/weapon/weapon1_3.png");
    //1-(x-1)^2


    public Weapon1_3Renderer(EntityRendererProvider.Context ctx) {
        super(ctx);
    }

    @Override
    public void render(Weapon1_3_Entity entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        long tick = entity.level().getGameTime();
        float ratio = (float) entity.getLife()/entity.getLifeMax();
        float r = 1-(ratio-1)*(ratio-1);
        renderSegment(poseStack, buffer, TEXTURE, partialTick,0.1F,tick,0.5f,r*3f);
    }
    public static void renderSegment(PoseStack poseStack, MultiBufferSource multiBufferSource, ResourceLocation texture, float partialTick, float rollingSpeed, long tick,float height,float length) {
        poseStack.pushPose();
    ...
    @Override
    public @NotNull ResourceLocation getTextureLocation(@NotNull Weapon1_3_Entity p_114482_) {
        return TEXTURE;
    }
}