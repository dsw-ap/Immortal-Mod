...
import com.tearhpi.immortal.client.textrender.GradientGlyphRenderer;
 ...
 */
@OnlyIn(Dist.CLIENT)
public class NameWithIconTooltipComponent implements ClientTooltipComponent {
    ...
    private final int weapon_level;
    private final int weapon_type;
    private final WeaponAttributeAttack weapon_attribute;
    private final Multimap<Holder<Attribute>, AttributeModifier> attributeModifiers;
    private final Multimap<Holder<Attribute>, AttributeModifier> attributeModifiers_base;
    private final LinkedList<String> weapon_skill_passive;
    private final LinkedList<String> weapon_skill_active;
    private final LinkedList<String> weapon_skill_attached;
    public final int attached_attribute_rune_max; //武器附加属性符文最大值
    public final int attached_attribute_rune; //武器附加属性符文
    public final int attached_skill_rune_max; //武器技能符文最大值
    public final int attached_skill_rune; //武器技能符文
    public final int weapon_enhancement; //强化等级
    public final int weapon_forging; //锻造等级
    public final int weapon_gilding; //镀金等级
    public final int weapon_forging_num_1; //锻造提升_暴击
    public final int weapon_forging_num_2; //锻造提升_造成伤害
    ...
    public NameWithIconTooltipComponent(ItemImageTooltipData data) {
        ...
        this.weapon_level = data.weapon_level();
        this.weapon_type = data.weapon_type();
        this.attributeModifiers = data.attributeModifiers();
        this.attributeModifiers_base = data.attributeModifiers_base();
        this.weapon_attribute = data.weaponAttributeAttack();
        this.weapon_skill_passive = data.weapon_skill_passive();
        this.weapon_skill_active = data.weapon_skill_active();
        this.weapon_skill_attached = data.weapon_skill_attached();
        ...
        this.attached_skill_rune_max = data.attached_skill_rune_max();
        this.attached_skill_rune = data.attached_skill_rune();
        this.weapon_enhancement = data.weapon_enhancement();
        this.weapon_forging = data.weapon_forging();
        this.weapon_forging_num_1 = data.weapon_forging_num_1();
        this.weapon_forging_num_2 = data.weapon_forging_num_2();
        this.weapon_gilding = data.weapon_gilding();
    ...
    public void renderImage(Font font, int x, int y, GuiGraphics gui) {
        ...
        if (this.weapon_level == 7) {
            ...
            rainbowColor.draw();
        } else if (this.weapon_level == 8) {
            GradientGlyphRenderer.drawGradientString(gui,font, originalName.getString(),textX, textY-3,0xFFFF9966,0xFFFF0000,0x00F000F0,true);
            //GradientGlyphRenderer.drawGradientString(gui,font, originalName.getString(),textX, textY,0xFF33cccc,0xFF3333cc,0x00F000F0,true);
        } else {
        ...
        gui.drawString(font, getWeaponLevelComponent().copy().append(getWeaponTypeComponent().copy().append(getWeaponAttackAttribute())), 0, 0, 0x555555);
        //第三行文字
        ...
        for (Map.Entry<Holder<Attribute>, AttributeModifier> entry : attributeModifiers_base.entries()) {
            // 获取 modifier 信息
            ...
            //匹配 绘制
            if (attributeId.toString().equals("immortal:attack_damage")) {
                gui.drawString(font, Component.translatable("tooltips.name.generic.immortal_attack_damage", String.format("%.1f",amount)), textX_AttributeRender, textY_AttributeRender, 0x55FFFF);
                textY_AttributeRender += 10;
            }
            if (attributeId.toString().equals("minecraft:generic.attack_speed")) {
                gui.drawString(font, Component.translatable("tooltips.name.generic.minecraft_attack_speed", String.format("%.1f",(4 + amount))), textX_AttributeRender, textY_AttributeRender, 0x55FFFF);
                textY_AttributeRender += 10;
            }
            //if (attributeId.toString().equals("minecraft:player.entity_interaction_range")) {gui.drawString(font, Component.translatable("tooltips.name.generic.minecraft_attack_range", amount), textX_AttributeRender, textY_AttributeRender, 0x55FFFF);}
        }
        textY_AttributeRender += 10;
        //未按下shift时 - 仅渲染基础属性
        if (!Screen.hasShiftDown()) {
            gui.drawString(font, Component.translatable("tooltips.press_shift"), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
            textY_AttributeRender += 10;
        } else {
            for (Map.Entry<Holder<Attribute>, AttributeModifier> entry : attributeModifiers.entries()) {
                ...
                ResourceLocation attributeId = entry.getKey().unwrapKey().map(ResourceKey::location).orElse(ResourceLocation.fromNamespaceAndPath("minecraft", "unknown"));
                double amount = entry.getValue().getAmount();
                if (attributeId.toString().equals("immortal:critical_chance")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.critical_chance", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:critical_damage")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.critical_damage", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:ignore_defense")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.ignore_defense", String.format("%.1f",amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:extra_mdamage")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.extra_mdamage", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:final_damage_add")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.final_damage_add", String.format("%.1f",amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("minecraft:generic.movement_speed")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.movement_speed", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:element_mdamage_fire")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.element_mdamage_fire", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:element_mdamage_water")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.element_mdamage_water", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:element_mdamage_earth")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.element_mdamage_earth", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:element_mdamage_air")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.element_mdamage_air", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:element_mdamage_light")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.element_mdamage_light", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:element_mdamage_darkness")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.element_mdamage_darkness", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:skill_cooldown_immune")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.skill_cooldown", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                if (attributeId.toString().equals("immortal:skill_range")) {
                    gui.drawString(font, Component.translatable("tooltips.name.player.skill_range", String.format("%.1f",100 * amount)), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
                }
                textY_AttributeRender += 10;
            }
        }
        ...
        //技能渲染
        if (!weapon_skill_passive.isEmpty() || !weapon_skill_active.isEmpty()) {
            for (String info : weapon_skill_passive) {
                gui.drawString(font, Component.translatable(info), textX_AttributeRender, textY_AttributeRender, 0xFF55FF);
                textY_AttributeRender += 10;
            }
            for (String info : weapon_skill_active) {
         ...
         */
        //强化渲染
        if (weapon_enhancement > 0){
            //强化等级渲染
            /*
            gui.drawString(font, Component.translatable("tooltips.attached_enhancement_1",weapon_enhancement,2*weapon_enhancement), textX_AttributeRender, textY_AttributeRender, 0xFF5555);
            textY_AttributeRender += 10;
            gui.drawString(font, Component.translatable("tooltips.attached_enhancement_2",weapon_enhancement,weapon_enhancement), textX_AttributeRender, textY_AttributeRender, 0xFF5555);
            textY_AttributeRender += 10;
             */
            gui.drawString(font, Component.translatable("tooltips.attached_enhancement",weapon_enhancement,2*weapon_enhancement,weapon_enhancement), textX_AttributeRender, textY_AttributeRender, 0xFF5555);
            textY_AttributeRender += 10;
        }
        //锻造渲染
        if (weapon_forging > 0){
            //强化等级渲染
            double num1 = (float) weapon_forging_num_1 / 100.0;
            double num2 = (float) weapon_forging_num_2 / 100.0;
            /*
            gui.drawString(font, Component.translatable("tooltips.attached_forging_1",weapon_forging,num1), textX_AttributeRender, textY_AttributeRender, 0xFFFF55);
            textY_AttributeRender += 10;
            gui.drawString(font, Component.translatable("tooltips.attached_forging_2",weapon_forging,num2), textX_AttributeRender, textY_AttributeRender, 0xFFFF55);
            textY_AttributeRender += 10;
             */
            gui.drawString(font, Component.translatable("tooltips.attached_forging",weapon_forging,num1,num2), textX_AttributeRender, textY_AttributeRender, 0xFFFF55);
            textY_AttributeRender += 10;
        ...
        if (weapon_gilding > 0){
            Component text = null;
            if (weapon_gilding == 1){
                text = Component.translatable("tooltips.attached_gilded_1",1,1,0);
            } else if (weapon_gilding == 2){
                text = Component.translatable("tooltips.attached_gilded_1",2,1,1);
            } else if (weapon_gilding == 3){
                text = Component.translatable("tooltips.attached_gilded_1",3,2,1);
            } else if (weapon_gilding == 4){
                text = Component.translatable("tooltips.attached_gilded_1",4,2,2);
            }
            gui.drawString(font, text, textX_AttributeRender, textY_AttributeRender, 0xFFAA00);
            textY_AttributeRender += 10;
        }
        //花形标识渲染
        if (weapon_enhancement > 10) {
            gui.drawString(font, Component.translatable("tooltips.attached_enhancement_3"), textX_AttributeRender, textY_AttributeRender, 0xFF5555);
            textY_AttributeRender += 10;
        }
        if (weapon_forging == 5) {
            gui.drawString(font, Component.translatable("tooltips.attached_forging_3"), textX_AttributeRender, textY_AttributeRender, 0xFFFF55);
            textY_AttributeRender += 10;
        }
        if (weapon_gilding == 4){
            gui.drawString(font, Component.translatable("tooltips.attached_gilded_2"), textX_AttributeRender, textY_AttributeRender, 0xAAAAAA);
            textY_AttributeRender += 10;
        }
    ...
    public int getHeight() {
        ...
        for (Map.Entry<Holder<Attribute>, AttributeModifier> entry : attributeModifiers_base.entries()) {text_line_count += 10;}
        if (Screen.hasShiftDown()) {
            for (Map.Entry<Holder<Attribute>, AttributeModifier> entry : attributeModifiers.entries()) {text_line_count += 10;}
        } else text_line_count += 10;
        if ((!weapon_skill_passive.isEmpty() || !weapon_skill_active.isEmpty())) {
            for (String info : weapon_skill_passive) {
                text_line_count += 10;
            }
        ...
        for (String info : weapon_skill_attached) {text_line_count += 10;}
        for (Component info : getRelatedLinkedList()) {text_line_count += 10;}
        text_line_count += 10;//Empty Line
        if (weapon_level == 7) {
    ...
    public int getWidth(Font font){
        int skill_width_max = Math.max(maxLengthInLinkedListString(weapon_skill_passive,font),maxLengthInLinkedListString(weapon_skill_active,font));
        int skill_width_max_ = Math.max(maxLengthInLinkedListString(weapon_skill_attached,font),skill_width_max);
        int skill_width_max__ = Math.max(maxLengthInLinkedListComponent(getRelatedLinkedList(),font),skill_width_max_);
        int width_max = Math.max(32 + 4 + font.width(originalName),Math.max(
                font.width(getWeaponLevelComponent().copy().append(getWeaponTypeComponent().copy().append(getWeaponAttackAttribute()))),Math.max(
                        skill_width_max__, font.width(Component.translatable("tooltips.press_shift")))
                )
        );
        if (weapon_level == 7) {
            return 11 + width_max;
    ...
    //获取武器种类
    private Component getWeaponTypeComponent() {
        if (this.weapon_type == 1) {
            return Component.translatable("tooltips.weapon_type_1");
        } else if (this.weapon_type == 2) {
            return Component.translatable("tooltips.weapon_type_2");
        }
        return null;
    }
    //获取武器攻击加成
    private Component getWeaponAttackAttribute() {
        ...
        } else if (this.weapon_attribute.getattributeName().equals("none")) {
            return Component.translatable("tooltips.weapon_attack_attribute_7");
        }
        return null;
    }
    //获取品质和武器名字渲染颜色
    ...
    private void renderFlowerText(GuiGraphics gui,Font font,int x,int y) {
        ...
        //附加武器
        for (int i=0;i<attached_skill_rune;i++) {
            gui.drawString(font, Component.literal("✼"), x, y, 0xFF5555);
            x += 8;
        }
        for (int i=0;i<attached_skill_rune_max-attached_skill_rune;i++) {
            gui.drawString(font, Component.literal("✼"), x, y, 0x555555);
            x += 8;
        }
        //强化
        ...
        //锻造
        if (weapon_forging == 5) {
            gui.drawString(font, Component.literal("❂"), x, y, 0xFFFF55);
            x += 8;
        } else {
            gui.drawString(font, Component.literal("❂"), x, y, 0x555555);
            x += 8;
        ...
        //镀金
        if (weapon_gilding > 3 ) {
            gui.drawString(font, Component.literal("✪"), x, y, 0xFFAA00);
            x += 8;
        } else {
            gui.drawString(font, Component.literal("✪"), x, y, 0x555555);
            x += 8;
    ...
    private LinkedList<Component> getRelatedLinkedList(){
        LinkedList<Component> list = new LinkedList<>();
        if (weapon_enhancement > 0) {
            //list.add(Component.translatable("tooltips.attached_enhancement_1",weapon_enhancement,2*weapon_enhancement));
            //list.add(Component.translatable("tooltips.attached_enhancement_2",weapon_enhancement,2*weapon_enhancement));
            list.add(Component.translatable("tooltips.attached_enhancement",weapon_enhancement,2*weapon_enhancement,weapon_enhancement));
            if (weapon_enhancement > 10) {
                list.add(Component.translatable("tooltips.attached_enhancement_3"));
            }
        }
        if (weapon_forging > 0){
            //强化等级渲染
            double num1 = Math.round((float) weapon_forging_num_1 /100);
            double num2 = Math.round((float) weapon_forging_num_2 /100);
            //list.add(Component.translatable("tooltips.attached_forging_1",weapon_forging,num1));
            //list.add(Component.translatable("tooltips.attached_forging_2",weapon_forging,num2));
            list.add(Component.translatable("tooltips.attached_forging",weapon_forging,num1,num2));
            if (weapon_forging == 5) {
                list.add(Component.translatable("tooltips.attached_forging_3"));
            }
        }
        if (weapon_gilding > 0){
            list.add(Component.translatable("tooltips.attached_gilded_1",1,1,1));
            if (weapon_gilding == 4) {
                list.add(Component.translatable("tooltips.attached_gilded_2"));
            }
        }
        return list;
    }
